\documentclass[12pt]{report}
    ...
    \begin{table}[h]
        ...
        \resizebox{\columnwidth}{!}{
            ...
            \hline
            Test Case ID & Test Scenario                                                                  & Test Steps                                                                                                                                                & Expected                                                                                                                 & Actual                                                                                 & Result & Comment \\ \hline
            FS001        & Test the image names                                                           & \begin{tabular}[c]{@{}l@{}}1. Fetch through\\ meme directory\\ 2. Check the \\ spelling\\ 3. Check the\\ enumeration\end{tabular}                         & \begin{tabular}[c]{@{}l@{}}isdigit() method to\\ return true and to \\ be picked up by an\\ assertion\end{tabular}       & \begin{tabular}[c]{@{}l@{}}Assertion returned\\ true\end{tabular}                      & Pass   &         \\ \hline
            FS002        & Test image integrity                                                           & \begin{tabular}[c]{@{}l@{}}1. Fetch through\\ image directory\\ 2. Find all files\\ ending with .jpg\\ 3. Open and\\ verify them using\\ PIL\end{tabular} & \begin{tabular}[c]{@{}l@{}}Assertion Boolean\\ variable to be set\\ to false if the\\ verification fails\end{tabular}    & \begin{tabular}[c]{@{}l@{}}Verification passed;\\ all images are\\ intact\end{tabular} & Pass   &         \\ \hline
            FS003        & \begin{tabular}[c]{@{}l@{}}Test availability of the\\ directories\end{tabular} & \begin{tabular}[c]{@{}l@{}}1. Specify paths\\ to the directories\\ 2. Test with an\\ assertion\end{tabular}                                               & \begin{tabular}[c]{@{}l@{}}The assertions to\\ be returned true\\ if the paths are\\ correct and dirs exist\end{tabular} & \begin{tabular}[c]{@{}l@{}}Dirs exist and the\\ paths are right\end{tabular}           & Pass   &         \\ \hline
            \end{tabular}%
        }
    \end{table}
    ...
    \subsubsection*{GUI}
    \addcontentsline{toc}{subsubsection}{GUI}
    \paragraph{}

    The GUI elements were tested using the user-side manual testing approach. The scrolling works okay, content is being loaded properly, although with a special button to load it. The branding section exists but it should be improved. Animation of the `load more' button works as it should.
    The function that hides the button when there's nothing from the image array to load works correctly.
